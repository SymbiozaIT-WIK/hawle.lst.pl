...
//    }
    ...
    public function create_wz_from_xml($xml){
        ...
        if(!$this->input->post('tempid')){
            ...
            if(isset($xml['SalesLine'][0]['KodLokalizacji'])){
                $headerMag              = $xml['SalesLine'][0]['KodLokalizacji'];
            }else if(isset($xml['SalesLine']['KodLokalizacji'])){
                $headerMag              = $xml['SalesLine']['KodLokalizacji'];
            }else{
                $headerMag = '';
            }
    ...
    //edycja headera
        ...
        if(!$this->input->post('tempid')){
            ...
            }else if(isset($xml['SalesLine']['Nr'])){
                $dbitem = $this->Xml_model->get_item_by_index(trim($xml['SalesLine']['Nr']));
                    if($dbitem){
                        $orderLine=array(
                            'itemcode' => $dbitem->code,
                            'regionalwarehousecode' => $xml['SalesLine']['KodLokalizacji'],
                            'quantity' =>$xml['SalesLine']['Ilosc'],
                            'tempdocumentno' => $wzId,
                            'description' => $dbitem->description
                        );
                        $this->Order_model->add_line($wzId,$orderLine);
